export default async function handler(req, res) {
  ...
  const { messages, systemPrompt, agentId, agentModel } = req.body;
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: agentModel || 'claude-haiku-4-5-20251001', max_tokens: 4096, system: finalSystemPrompt, messages }),
    });
